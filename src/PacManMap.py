...
from games.PacMan.src.PacPlayer import PacPlayer
        ...
        for layer in self.tmxdata.visible_layers:
            ...
            for x, y, gid, in layer:
                if isinstance(layer, pytmx.TiledTileLayer):
                    if gid != 0:  # 0代表空格，無圖塊
                        if layer.parent.tiledgidmap[gid] in WALLS_IMG_PATH_DIC:
                            self.wall_no += 1
                            wall = PacWall(self.wall_no, layer.parent.tiledgidmap[gid],
                                           x * TILE_X_SIZE, y * TILE_Y_SIZE)
                            self.walls.append(wall)
                        elif layer.parent.tiledgidmap[gid] == POWER_PELLET_IMG_NO_LIST:
                            power_pellet = PowerPellet(x * TILE_X_SIZE, y * TILE_Y_SIZE)
                            self.power_pellets.append(power_pellet)
                        elif layer.parent.tiledgidmap[gid] == DOT_IMG_NO_LIST:
                            ...
                            self.player = PacPlayer(x * TILE_X_SIZE, y * TILE_Y_SIZE)
