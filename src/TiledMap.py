...
class Construction:
    def __init__(self, _id: int or str, _no: int, _init_pos: tuple, _init_size: tuple):
        self.id = _id
        self.no = _no
        self.init_pos = _init_pos
            ...
            for x, y, gid, in layer:
                if isinstance(layer, pytmx.TiledTileLayer):
                    ...
                    if not self._is_record and not gid:  # 0代表空格，無圖塊
                        ...
                        obj_no += 1
                        img_info = Construction(_id=img_id, _no=obj_no
                                                , _init_pos=pos, _init_size=(self.tile_width, self.tile_height)
                                                )
